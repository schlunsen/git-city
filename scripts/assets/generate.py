...
JOBS = {
    ...
    # lots-4 .. lots-7 (LOT_KINDS lawn, paved, tennis, soccer in public/city/lots.js):
    'lots2': ('1:1', f'2x2 grid of four square top-down orthographic cartoon city lot tiles, the tiles separated ONLY by plain flat magenta gaps: 1) plain mowed green lawn with a winding bare footpath, 2) paved pedestrian plaza with neat concrete pavers in a subtle pattern, 3) tennis court with green surface, white painted lines and a net across the middle, 4) small soccer pitch with green grass, white markings and a goal at each end, {STYLE}, no numbers, no labels, no grid lines, no crosshair lines, {MAG}'),
    # lots-8 .. lots-11 (playground, garden, market, fountain):
    'lots3': ('1:1', f'2x2 grid of four square top-down orthographic cartoon city lot tiles, the tiles separated ONLY by plain flat magenta gaps: 1) children playground with a slide, a swing set and a sandbox seen from directly above, 2) community garden with neat rectangular vegetable beds and mulch paths, 3) market square with rows of little striped market stalls around an open paved plaza, 4) fountain plaza with a round central fountain and radiating paved paths, {STYLE}, no numbers, no labels, no grid lines, no crosshair lines, {MAG}'),
    # lots-12 .. lots-15 (skate, dog-park, track, amphitheater):
    'lots4': ('1:1', f'2x2 grid of four square top-down orthographic cartoon city lot tiles, the tiles separated ONLY by plain flat magenta gaps: 1) skate park with concrete ramps, a quarter pipe and grind rails seen from directly above, 2) fenced dog park with grass, a dirt path and a small agility hurdle, 3) athletics running track with red oval lanes around an empty green field, no goals, no flags, 4) small open-air amphitheater with curved wooden bench rows facing a round stage, {STYLE}, no numbers, no labels, no grid lines, no crosshair lines, {MAG}'),
    # lots-2 (construction) redraw: the original grid tile was nearly bare. A single
    # full-frame tile, keyed with `key.py row ... --name lots --start 2` (1 sprite).
    # "Strict top-down" is stressed hard or the model drifts to isometric blocks.
    'construction2': ('1:1', f'A single square cartoon construction site tile seen from DIRECTLY ABOVE, strict top-down orthographic view, like a flat map tile, no perspective, no isometric angle, no visible sides, edges or depth, completely flat 2D, the tile fills the whole frame: excavated sandy ground with a deep rectangular foundation pit, a yellow tower crane seen from above, stacks of steel beams and concrete pipes, piles of sand, a small cement mixer, orange traffic cones and striped orange-and-white barriers, {STYLE}, no numbers, no labels, {MAG}'),
    ...
    # props-6 .. props-9 = PROP.BUS_SHELTER, TRASH_BIN, NEWS_STAND, BIKE_RACK. Glass must be opaque (see docs/assets.md rule 9).
    'street_furniture': ('16:9', f'Sprite sheet of 4 different cartoon city street-furniture objects in a single row, side view, evenly spaced with clear gaps: a small steel bus shelter with a flat roof, a bench inside and opaque pale-blue frosted glass panels, a green metal trash bin with a swing lid, a red newspaper stand kiosk with a small window, a silver metal bike rack shaped like a rounded rectangle, no ground shadows, {STYLE}, {MAG}'),
}
