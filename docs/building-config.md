...
In [Git City](https://schlunsen.github.io/git-city/) every repository is a
...
```json
{
  "$schema": "https://schlunsen.github.io/git-city/schema/building-config.v1.json",
  "version": 1,
...
Text is free-form (any language, emoji, punctuation). Git City trims it,
...
## When Git City reads it

- Only for buildings that are on screen: when a city loads, Git City checks
...
- A missing file (404) means no config. The browser may log each 404 in its
  console, which is why Git City keeps these requests few.
...
- Text is capped and only ever painted onto a canvas with `fillText`, so it
  can't become HTML.
- No field is a URL, and Git City never loads anything a config names.
