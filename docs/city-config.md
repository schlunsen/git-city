...
| `island.streets` | number | `3`–`5` (default `4`) | Width of the inner streets, in world units. Wider streets leave a smaller lot, so the buildings standing on them are slimmer. Narrower ones turn the extra room into pavement rather than bigger buildings, so your city always keeps the same footprint. |
